...
void List::shuffle() {
   std::shuffle(&m_move[0], &m_move[m_size], ml::gen);
}
...
// prototypes
   ...
   static int    best_score(const List & list);
   static double total(const List & list, int bs, double k);

   static double weight(int sc, int bs, double k);
   ...
   int pick(const List & list, double k) {
      ...
      for (int i = 0; i < list.size(); i++) {
         ...
         double w = weight(list.score(i), bs, k);
         tot += w;
   ...
   void disp(const List & list, const Pos & pos, double k) {

      assert(list.size() != 0);

      int bs = best_score(list);
      double tot = total(list, bs, k);
      ...
      for (int i = 0; i < list.size(); i++) {
         ...
         std::string string = move::to_string(mv, pos);
         double p = weight(sc, bs, k) / tot;

         std::printf("%s %+d (%.1f%%)\n", string.c_str(), sc, p * 100.0);
      }
   ...
   static double weight(int sc, int bs, double k) {
      assert(sc <= bs);
      return std::exp(double(sc - bs) / 100.0 * k);
   }
